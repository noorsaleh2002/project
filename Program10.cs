...
namespace part2_umlProject
{
    ...
    class System
    {
        ...
        public List<Listing> LoadSellersItem()
        {
            try
            {
                FileStream fileStream = new FileStream("SellersItem.txt", FileMode.Open, FileAccess.Read);
                BinaryFormatter formatter = new BinaryFormatter();
                List<Listing> l = new List<Listing>();
                int i = 0;
                while (fileStream.Position < fileStream.Length)
                    l[i++] = formatter.Deserialize(fileStream) as Listing;
                fileStream.Close();
                return l;
            }
        ...
        public void SaveSellersListing(List<Listing> l)
        {
            FileStream fileStream = new FileStream("SellersItem.txt", FileMode.Append, FileAccess.Write);
            BinaryFormatter formatter = new BinaryFormatter();
            for (int i = 0; i < l.Count; i++)
            {

                formatter.Serialize(fileStream, l[i]);
            }
            fileStream.Close();
        }
                    ...
                    { //Adding new listing:
                      // Seller[] s1 = LoadSellersItem();
                        ...
                        while (flage)
                        {
                            ...
                            if (more != "yes")
                                flage = false;
                            SaveSellersListing(s.GetlistItime());
                        }
                    }
                    if (chose == 2)
                    {//Deleting exist listing 
                        List<Listing> l = LoadSellersItem();
